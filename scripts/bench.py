...
size of the compiled test program, so small crates run much faster than one large crate, with
identical numbers.

One snapshot file per bench module (`bench_trig.cairo` -> `gas/trig.snap`) so that parallel pull
