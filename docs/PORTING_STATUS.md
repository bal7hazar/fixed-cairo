...
| F6 | correctly rounded `div_nearest` / `recip_nearest` / `RecipTrait::div_nearest` (nalgebra-cairo escalation, 2026-09-23); `/`, `recip`, `from_ratio` switched to round-half-even (Rust reference; +10.7 % on `/`, 173 benches in 18 snapshots); release `v0.3.0` | inreview | #42 |
